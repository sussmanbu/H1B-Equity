...
```
```{r}
percentage_with_software <- filtered_data %>%
  mutate(contains_software = str_detect(tolower(JOB_TITLE), "software")) %>%
  summarise(percentage = mean(contains_software) * 100)

percentage_with_engineeer <- filtered_data %>%
  mutate(contains_engineer = str_detect(tolower(JOB_TITLE), "engineer|developer")) %>%
  summarise(percentage = mean(contains_engineer) * 100)

percentage_with_software
percentage_with_engineeer
```
...
ggplot(top_states, aes(x = reorder(EMPLOYER_STATE, -Total_Applications), y = Total_Applications)) +
  ...
  labs(title = "Total Visa Applications by State, 2023", x = "State", y = "Number of Applications") +
  coord_flip()
```
...
yearly_wage <- final_df %>%
  group_by(YEAR_BEGIN) %>%
  summarise(WAGE_RATE_OF_PAY_FROM = median(WAGE_RATE_OF_PAY_FROM, na.rm = TRUE))

ggplot(yearly_wage, aes(x = YEAR_BEGIN, y = WAGE_RATE_OF_PAY_FROM)) +
  geom_line() +  # Use a line plot to show trends over time
  geom_point() +  # Add points to highlight each year's average wage
  labs(title = "Median Employer wage Per Year",
       x = "Year",
       y = "Median employer wage") 
