...
```{r}
new_summary_data <- final_df |>
  group_by(YEAR_BEGIN, MONTH_BEGIN) |>
  summarise(total_cases = n_distinct(CASE_NUMBER))

# Creating a new combined column for plotting
new_summary_data$YearMonth <- as.Date(paste(new_summary_data$YEAR_BEGIN, new_summary_data$MONTH_BEGIN, "01", sep="-"))

# Plotting the data
ggplot(new_summary_data, aes(x = YearMonth, y = log(total_cases) )) +
...
```{r}
df16 <- readRDS("dataset/2016H1B.rds")
df16 <- df16 |>
  select("CASE_NUMBER", "EMPLOYMENT_START_DATE")

```

```{r}
df17 <- readRDS("dataset/2017H1B.rds")
df17 <- df17 |>
  select("CASE_NUMBER", "EMPLOYMENT_START_DATE")
...
```{r}
df18 <- readRDS("dataset/2018H1B.rds")
df18 <- df18 |>
  select("CASE_NUMBER", "EMPLOYMENT_START_DATE")
...
```{r}
combined_df <- rbind(df16, df17, df18)
save()
```

```{r}
combined_df$YEAR_BEGIN = as.integer(substr(combined_df$EMPLOYMENT_START_DATE, 1, 4))
combined_df$MONTH_BEGIN = month(combined_df$EMPLOYMENT_START_DATE)
```
...
# Prepare the data
model_df <- new_summary_data |>
  ungroup() |>
  select(YearMonth, total_cases)
...
# algorithm by facebook
df <- data.frame(ds = model_df$YearMonth, y = model_df$total_cases)
m <- prophet(df)
...
# Convert 'ds' to POSIXct if not already
forecast$ds <- as.POSIXct(forecast$ds, format = "%Y-%m-%d")
df$ds <- as.POSIXct(df$ds, format = "%Y-%m-%d")

# Plot with ggplot2
gg <- ggplot() +
  geom_line(data = forecast, aes(x = ds, y = yhat), color = "blue") +
  geom_ribbon(data = forecast, aes(x = ds, ymin = yhat_lower, ymax = yhat_upper), fill = "blue", alpha = 0.2) +
  geom_point(data = df, aes(x = ds, y = y), color = "red") +
  labs(x = "Date", y = "Total Cases", title = "Forecast of Total Cases")
