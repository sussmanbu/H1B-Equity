...
```
#List of columns
Some years may differ, we will filter them out

```{r}
colnames(fy23)
```
```{r}
sum(is.na(fy19$FOREIGN_WORKER_INFO_EDUCATION))
```
...
education <- combined_19_23 |>
  ...
  summarise(sum_accepted = n())
```

```{r}
ggplot(education, aes(y = sum_accepted, x = yearmonth(year_month), color = FOREIGN_WORKER_EDUCATION)) +
  geom_line() +
  labs(x = "Decision Date", y = "Sum Accepted", title = "Applicants and Education Levels")
```
