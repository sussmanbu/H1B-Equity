...
population_data$Year <- as.character(population_data$Year)
...
source("scripts/standardize_country_names.R")
...
reds_palette <- brewer.pal(9, "Reds")
...
p <- ggplot() +
  ...
  scale_fill_gradientn(colors = reds_palette) +
  labs(title = "H1B Visa Distribution 2022, Normalized by Population")+
...
mergetry$Population <- as.numeric(gsub(",", "", mergetry$Population))
...
```{r}
ggplot(mergetry_2023, aes(x = num_applicants/Population, y = num_certified/Population)) +
  geom_point(shape = 21, fill = "blue", color = "black", size = 3, alpha = 0.5) +
...
```{r}
linear_model_pop <- lm(num_applicants/Population ~ num_certified/Population -1, data = mergetry_2023)
...
```{r}
ggplot(mergetry_2023, aes(x = Population, y = num_applicants)) +
  geom_point(shape = 21, fill = "blue", color = "black", size = 3, alpha = 0.5) +
