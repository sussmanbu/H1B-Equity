...
Our data analysis seeks to understand how H1-B applications vary across countries, continents, and time. More specifically, we are interested in the number of applications from each country and/or continent, the wages of the recipients, the level of education that the applicants have achieved, and the economic data of their country of citizenship. We want to investigate how the distribution of H1-B applicants per continent has changed over time, and how (if at all) wage is linked to education level, GDP per capita of country of origin, and Unemployment rate of country of origin for applicants with successful H1B visa applications.
...
First, the Perm Disclosure datasets for 2023 and 2024 are joined with IMF GDP and Unemployment rates by country.

The education level of an applicant is made into a categorical variable. To feed into the linear regression model, we need to encode into numeric columns using tidyverse.

The correlation between variables can be represented in the following correlation matrix. ![](images/correlation_heatmap.png)

With the information from this correlation matrix, an initial linear regression model of Wage \~ GDP per Capita + Unemployment can be built.

To build our linear regression model, these features are used
...
The reason to select these features among the many is to predict wage based on the education level. For instance, employers might be more likely to select H1B workers who attained higher education level since H1B job positions is limited. Employers would want to find strong hires to fill the position.

A naive implementation of the model would be strongly influenced by an outlier in the data, which can be detected using Cook's distance formula. Nigeria, which has an average H1-B applicant wage of \$6,524,999, a GDP of 1945.012, and an unemployment rate of 22.6, has a Cook's distance more than 3 times the average.
...
The model's predictive performance of the testing set has a RSME of 48892.1392, due to the assumption that linear regression assumes the data to have linear relationship, which is why we have such a high error.

However, in real setting, most of the real world dataset is nonlinear. To obtain a lower root square mean error, we would need a robust machine learning model or transform some of the features.![Total number of H1B applications in 2020-2024](images/ActualvsPredicted.png)
...
![Total number of H1B applications in 2020-2024](images/total_cases.png)
...
3.  To improve on the model performance, that is, predict on the wage, machine learning models or neural network can be used to minimize the error.
